                                    ...
                                    <div class="col-sm-6 m-t-20">
                                        ...
                                        <select class="form-control select2" name="report-for-user" id="report-user">
                                            ...
                                            @foreach($usersList as $userRec)
                                                <option value="{{ $userRec['id'] }}">{{ $userRec['first_name'] }} {{$userRec['last_name'] }} -
                                                    {{ $userRec['email'] }}</option>
                                            @endforeach
                                        </select>
    ...
    {{--                                    </form>--}}
                                ...
                                <div class="box-footer">
                                    <div id="saveActions" class="form-group">
                                        <div class="btn-group">
                                            <button type="submit" class="btn btn-success save-action">
...
// return;
                formData.append('send', 'save-report');
    ...
    <script>
        ...
        $('#drop-files').bind('drop', function(e) {
           ...
           var fileCheck =  isFile();
            ...
            if(!fileCheck){
                swal({
                    title: "",
                    text: "Invalid Format, Please upload CSV, PDF, DOC or DOCX, PNG, JPG, GIF format file only.",
                    type: 'error',
                    allowOutsideClick: false,
                    html: true,
                    showCancelButton: false,
        ...
        $('#uploadCustomersCSVFile').on('fileselect', function (event, numFiles, label) {
            ...
            if (log) {
                if ($.inArray(ext, ['csv', 'pdf', 'doc', 'docx', 'jpg', 'png', 'gif']) == -1) {
                    swal({
                        title: "",
                        text: "Invalid Format, Please upload CSV, PDF, DOC or DOCX, PNG, JPG, GIF format file only.",
                        type: 'error',

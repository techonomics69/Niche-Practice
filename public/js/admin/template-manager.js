...
var TOPOL_OPTIONS = {
            ...
            items: [
                {
                    ...
                    value: '<span class="template-token-tag" data-token="Doctor_Logo_Container"><a href="%%Doctor_Website_Url%%"><img style="max-width:250px; max-height: 60px;" class="template-token-tag" data-token-name="Doctor_Logo" src="'+docLogo+'" /></a></span>',
                    text: "Doctor Logo",
                },
                {
                    ...
                    value: '<span class="template-token-tag" data-token="Doctor_Photo_Container"><img style="max-width:140px; max-height: 160px;" class="template-token-tag" data-token-name="Doctor_Photo" src="'+docPhoto+'" /></span>',
                    text: "Doctor Photo"
                },
                {
                ...
                // {
                //     value: '<a href="tel:713-992-0916">1515181884</a>',
                //     text: "Doctor Phone2",
                // },
                {
                    value: "%%Doctor_Name%%",
                ...
                {
                    // value: '<span class="template-token-tag" data-token-name="Doctor_Unsubscribe_handler"><a href="%%Doctor_Unsubscribe_Url%%">%%Unsubscribe%%</a></span>',
                    value: "%%Unsubscribe%%",
                    text: "Unsubscribe",
                },
                {
                    value: '<span class="template-token-tag" data-token="Visit_Website_Container"><a href="%%Doctor_Website_Url%%">Visit Our Website</a></span>',
                    text: "Visit Our Website"
                },
                {
                    value: '<span class="template-token-tag" data-token="Email_Us_Container"><a href="mailto:%%Doctor_Personal_Email%%">Email Us</a></span>',
                    text: "Email Us"
                }
            ]
        }
    ...
    callbacks: {
        ...
        onSave: function(json, html) {
            ...
            if(window.ActionTarget == 'screenshot')
            {
        ...
        onBlockSave(json)
        {
            $(".save-block-interface small").html("");
            $(".save-block-interface .modal-title").remove();
            window.blockJson = json;
            window.blockId = '';
            ...
            $(mainModel).addClass('save-block-interface');
            ...
            html += '<input id="blockName" type="text" class="form-control input-lg" data-required="true" data-message="Invalid URL">';
            html += '<span class="help-block save-block-error"><small style="color:red; float: left;"></small></span>';
            html += '</div>';

            // html += '<input type="hidden" id="blockJson" value="'+json+'" />';
            html += '</div>';
            ...
            html += '<button type="submit" class="btn btn-primary save-block">Save</button>';
            html += '</div>';

            mainModel.modal('show');
            $(".modal-header", mainModel).append('<h4 class="modal-title">Save block</h4>');
            ...
            //     console.log("json de");
            //     console.log(json);
            //     console.log("json de 1");
            //     console.log(json.definition);
            //
            //     saveBlock(name, json.definition, window.templateId);
            // }
        },
        onBlockRemove(id) {
        ...
        onBlockEdit(id) {
            var blockName = listTemplate[id];
            blockId = id;
            // var name = window.prompt('Block name:', blockName);
            // if (name !== null) {
            ...
            html += '<h3 class="action-title"></h3>';
            html += '<div class="form-group">';
            html += '<input value="'+blockName+'" id="blockName" type="text" class="form-control input-lg" data-required="true" data-message="Invalid URL">';
            html += '<span class="help-block save-block-error"><small style="color:red; float: left;"></small></span>';
            html += '</div>';
            ...
            html += '<div class="modal-footer">';
            html += '<button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>';
            html += '<button type="submit" class="btn btn-primary save-block">Update</button>';
            html += '</div>';

            mainModel.modal('show');
            $(".modal-header", mainModel).append('<h4 class="modal-title">Update block</h4>');
            $(".modal-header", mainModel).after(html);
...
$(document.body).on('click', '.save-block', function(e)
{
    var blockName = $("#blockName").val();

    if(blockName !== '')
    {
        var json = window.blockJson;
        // console.log("blockJson");
        // console.log(blockJson);
        saveBlock(blockName, blockJson.definition, window.templateId, window.blockId);
    }
    else
    {
        $(".save-block-error small").html('Required field');
    }
});
...
function saveBlock(name, json = '', templateId = '', blockId = '') {
    ...
    data = {
        send: 'template-save-block',
        name: name,
        json: json,
        block_associated_template: templateId,
        blockId: blockId
    };
...
function deleteSavedBlock(id) {
    ...
    data = {
        send: 'template-delete-save-block',
        id: id
    };
...
function saveTemplate(json, html, templateId) {
    ...
    var niche = $("#niche").val();
    var showInDashboardVal = $('#show-in-dashboard').is(':checked');
    var showInDashboard;
    if(showInDashboardVal) {
        showInDashboard = 1;
    }
    else{
        showInDashboard = 0;
    }
    // console.log('showToPaid');
    // console.log(showToPaid);
    // console.log('showToPaid');
    var templateSource = $("#template-source").val();
    var date = $("#date").val();

    var category = $("#category").val();
    var type = $("#type").val();

    var campaignLinkedWIth =$("#campaign-user").val();
    ...
    data = {
        ...
        date: date,
        category: category,
        campaign_for_user: campaignLinkedWIth,
        title: title,
    ...
    $.ajax({
        ...
        if(data.id && data.id !== '')
        {
            ...
            if(template !== templateId)
            {
                ...
                if(templateSource === 'patient_campaign')
                {
                    clean_uri = clean_uri + 'templates/patient-email-template/'+template;
                }
                else
                {
        ...
        if(statusCode === 200)
        {
            ...
            console.log("window.ActionTarget");
            console.log(window.ActionTarget);
...
function saveScreenshot() {
    ...
    var canvas = $("#previewImage canvas")[0];

    if(isEmptyValNormal(canvas) == false)
    {
        // console.log("canvas");
        // console.log(canvas);
        ...
        formData.append("attach_screenshot[0]", blob);
        ...
        $.ajax({
            ...
            if(statusCode == 200)
            {
                ...
                if(data.screenshot != '')
                {
                    $(".template-screenshot").show();
                    $(".template-screenshot").html('<img src='+data.screenshot+'?uix='+timestamp+ '/>');
                }
            }
            else
            {
                if(screenshotCounter == 2)
                {
                    saveScreenshot();
                }
                else{
                    ...
                    swal({
                        title: "",
                        text: "Unable to take screenshot. Please try again.",
                        type: "error"
                    }, function () {});
                }
            }
...
function reloadSavedBlock() {
    ...
    $.ajax({
        ...
        $.each(data, function (index,value) {
            ...
            listTemplate[value.id] = value.name;
        });
    ...
    if(templateId && templateId !== '')
    {
        ...
        $.ajax({
            ...
            if(data.id && data.id !== '') {
                ...
                var showInDashboard = data.show_in_dashboard;
                ...
                var campaignLinked = data.campaign_for_user;
                ...
                $("#credits").val(credits);
                $("#title").val(title);
                // $("#show-in-dashboard").val(showInDashboard);

                if (showInDashboard)
                {
                    $( "#show-in-dashboard").prop('checked', true);
                }
                else
                {
                ...
                if(campaignLinked && campaignLinked !== '')
                {
                    $("#campaign-user").val(campaignLinked);
                    $("#campaign-user").change();
                }
                ...
                if(isEmptyValNormal(date) == false)
                {
                    $("#date").val(date);
                    $("#date").change();
                }
                ...
                if(industry && industry !== '')
                {
                    ...
                    // console.log("industryNiche");
                    // console.log(industryNiche);

                    // if(industryNiche && industryNiche !== '')
                    if(isEmptyValNormal(industryNiche) == false)
                    {
                ...
                if(isEmptyValNormal(screenshot) == false)
                {
                    ...
                    $(".template-screenshot").show();
                    $(".template-screenshot").html('<img src='+screenshotUrl+'?uix='+timestamp+ '/>')
                }

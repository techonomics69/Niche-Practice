...
$(function () {
    ...
    var promotionDataUserId  = window.promotionDataUserId;
    var UserId  = window.userId;
console.log('promotionDataUserId')
console.log(promotionDataUserId)
console.log('UserId')
console.log(UserId)
    // var siteUrl = $("#hfBaseUrl").val();
        ...
        if (selectedIndustry !== '') {
            ...
            $.ajax({
                ...
                if (statusCode == 200) {
                    ...
                    if (data) {
                        var html = '';
                        // html += '<option value="">Select Niche</option>';
                        html += '<option value="0">All Selected</option>';
    ...
    var pixie = new Pixie({
        ...
        onLoad: function() {
            ...
            // $(".open-button").before('<button id="saveJson" class="btn btn-default" style="display: none;padding-left: 30px;padding-right: 30px; margin-right: 16px;">Save Json Template</button>');
            // $(".open-button").before('<button id="saveUrl" class="btn btn-default" style="display: none;padding-left: 30px;padding-right: 30px; margin-right: 16px;">Copy Promotion Url</button>');
        ...
        onSave: function(data, name) {
            ...
            // if(promotionDataUserId  ) {
            //     if(UserId){
            //         if (promotionDataUserId != UserId ) {
            //             // swal({'Error', 'Admin can view only.','Error'})
            //             swal("Error", "You cannot edit Guest User Promotion.", "error");
            //         }
            //         else {
                        var state = pixie.getState();
...
$(document.body).on('click', '#saveJson', function () {
    var siteUrl = $("#hfBaseUrl").val();
...
$(document.body).on('click', '#saveUrl', function () {
    var siteUrl = $("#hfBaseUrl").val();
    var $temp = $("<input>");
    // var $url = $(location).attr('href');
    var $url = window.templateId;

    $("body").append($temp);
    $temp.val(siteUrl +'/create-promotion/'+ btoa($url)).select();
    document.execCommand("copy");
    swal({
        title: "Success",
        text: 'Promotion URL Copied!',
        type: "success"
    }, function () {
    });
    $temp.remove();

...
currentPageView = $("#currentPage").val();
        ...
        if (isRequired === "true") {
            ...
            if (currentFieldvalue === '') {
                ...
                if (ID === 'password' || ID === 'password-confirm') {
                    ...
                    if($("#password-confirm").length >= 1)
                    {
                        var primaryPassword = $('#password');
                        var passwordValue = primaryPassword.val();
                        ...
                        else if(passwordValue.length >= 8 && !strRegEx.test(passwordValue))
                        {
                                $(this).parent().find('span.help-block small').html('Please follow the password pattern.');
                                window[errorStatus] = true;
                        }
                    ...
                    {
                        if($(this).val().length < 8 && (currentPageView != 'login'))
                        {
                            $(this).parent().find('span.help-block small').html('Minimum 8 characters');
                            window[errorStatus] = true;
                        }
    ...
    $(document.body).on('submit', 'form'+targetForm+'', function (e) {
        ...
        $(''+targetForm+' input, '+targetForm+' textarea, '+targetForm+' select').each(function () {
            ...
            if (isRequired === "true") {
                ...
                if ((this.value === '')) {
                    $(this).blur();
                } else if ((this.value !== '' && ID === 'email') || (this.value !== '' && ID === 'password')) {

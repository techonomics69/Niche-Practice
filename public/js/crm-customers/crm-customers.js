...
emailContentMessage = 'We are committed to offering the highest level of patient care. Did our staff achieve that for you today? If so, please consider leaving us a review.';

smsContentMessage = 'We are committed to offering the highest level of patient care. Did our staff achieve that for you today? If so, please consider leaving us a review.\n';
...
function loadAddSingleCustomerStep2Settings(){
    ...
    {
        $("#addCustomerStep2 .modal-body").append('<div class="validate-check col-sm-12">\n' +
    ...
    if(typeof(customize_email)=='undefined' || customize_email==null || customize_email==''){
        ...
        // $('#email-preview-wrap .content-body').html('<p class="email-content" id="email-content">'+emailContentMessage+
        ...
        $('#email-content').html(customize_email);
    }
    ...
    if(typeof(customize_sms)=='undefined' || customize_sms==null || customize_sms==''){
        ...
        $('#sms-preview-wrap .content-body').html('' +
            '<p class="sms-content" id="sms-content">'+smsContentMessage+'</p>' +
    ...
    if(currentPage === 'home')
    {
        $('#enable_get_reviews').val(enableGetReviews);
        $('#sending_option').val(sendingOption);
        $('#smart_routing').val(smart_routing);
        $('#review_site').val(review_site);
        $('#send_reminder').val(reminder);
    }
    ...
    // when loading dom by step 2 review request
    displayCharacters("#email-preview-wrap #email-content", 'Add a Quick Review');
    displayCharacters("#sms-preview-wrap #sms-content", 'http://bit.ly/2LhkWmX');
...
$(document).ready( function () {
    ...
    if(currentPage !== 'email_campaign_builder' && currentPage !== 'home')
    {
        $.fn.dataTableExt.sErrMode = 'none';
    }


    $('#send_review_invite_view_report_link').click(function (event) {
        event.preventDefault();

        $('#addCustomerStep2').modal('show');

        $('#addCustomerStep2').addClass('crm-settings-modal')

        $('.st-1-header, .st-2-header, .st-3-header').hide();

        // $('#home-modals-wrapper #sending_option_panel').removeClass('hide');
        // $('#home-modals-wrapper #smart_routing_panel').removeClass('hide');
        // $('#home-modals-wrapper #send_reminder_panel').removeClass('hide');
        // $('#review_site_panel').removeClass('hide');
        // $('#home-modals-wrapper #preview_panel').removeClass('hide');


        loadAddSingleCustomerStep2Settings();

        // when modal open text limit apply
        displayCharacters("#email-preview-wrap #email-content", 'Add a Quick Review');
        displayCharacters("#sms-preview-wrap #sms-content", 'http://bit.ly/2LhkWmX');

        var viewedSendReviewInviteSettings = $('#viewedSendReviewInviteSettings').val();
        // console.log('viewedSendReviewInviteSettings');
        // console.log(viewedSendReviewInviteSettings);
        if (!(viewedSendReviewInviteSettings == 1)) {
            var baseUrl = $('#hfBaseUrl').val();
            $.ajax({
                headers: {
                    'X-CSRF-TOKEN': $('input[name="_token"]').val()
                ...
                type: "POST",
                url: baseUrl + "/viewedSendReviewInviteSettings",
                data: {
                    'viewed_send_review_invite_settings': '1'
                }
            }).done(function (data) {
                $('#viewedSendReviewInviteSettings').val(1);
                // console.log(data);
            }).fail(function (error) {
                // console.log(error);
            });
    ...
    customersListTable = $('#customers-list').DataTable({
        ...
        drawCallback: function( settings ) {
            ...
            if(val==''){
                $('.customers-header').html('Patients List'
                    // +
                    // ' <a class="page-help" href="javascript:void(0)"><i class="fa fa-question-circle-o" ' +
    ...
    function showError(targetElement, message = '') {
        ...
        if (currentPage == 'home') {
            targetElement.next('span').find('small').html("<small>"+'not empty'+"</small>");
            $('#send-review-invite-error-block').css("display", "block");
            $('#send-review-invite-error-message').text('Provide a phone number and/or email address');
        } else {
            ...
            // targetElement.next('span').css('visibility','hidden');

            $("#addCustomerStep1 .error_Msg").removeClass('hide-me');
            $("#addCustomerStep1 .error_Msg").addClass('error');
            $("#addCustomerStep1 .error_Msg").addClass('error-global-me');
            $("#addCustomerStep1 .error_Msg small").html('Please enter email address,  cell phone number or both.');
        }
    ...
    function hideError(targetElement) {
        ...
        if (currentPage == 'home') {
            ...
            $('#send-review-invite-error-block').css("display", "none");
            $('#send-review-invite-error-message').text('');
        } else {
    ...
    function addSingleCustomer() {
        ...
        var sending_option= reviewRequestSettingsData.sending_option;
        ...
        var baseUrl = $('#hfBaseUrl').val();
        var userUpgradeStatus = $('#userUpgradeStatus').val();

        if(sending_option == 2 && userUpgradeStatus == 'trial')
        {
            swal({
                    title: "SMS Quick Invites",
                    text: "Instantly invite your patients to leave a review for your dental services.",
                    type: "success",
                    confirmButtonText: "Upgrade Now",
                    customClass: "close-icon-alert"
                },
                function(inputValue){
            ...
            $(".close-icon-alert").prepend('<span class="close-icon">X</span>');
    ...
    function addSingleCustomerStep2() {
        ...
        else if(requestType=='extCustomers'){
        ...
        else
        {
            ...
            if($(".validate-check #checkbox-signup").is(':checked') != 1)
            {
                ...
                $('.validate-check').append('<span class="error help-block" style=""><small>Please check this box.</small></span>');
                return false;
            }
        ...
        $.ajax({
            ...
            success: function (response, status) {
                ...
                var records = response.records;
                // console.log('records')
                // console.log(records['_metadata']['message'])
                ...
                if(statusCode==200){
                    ...
                    if(requestType=='extCustomers'){
                        ...
                        // console.log('here');
                        // console.log(records['_metadata']['outcomeCode']);
                        var type = '';
                        if(records['_metadata']['outcomeCode'] == 200) {
                            type = 'success';
                        }
                        else {
                            type = 'error';
                        ...
                        swal({
                            ...
                            text: '' + records['_metadata']['message'],
                                // '<div class="notification-summary" style="margin-top: 30px;">' +
                                // '<p style="margin-bottom: 10px;"><i class="fa fa-check" aria-hidden="true"></i> <span>Contact added. </span></p>'+
                                // textMsg+
                                // '</div>',
                            ...
                            if(currentPage === 'home')
                            {
                                $("#addCustomerStep1 #first_name").val('');
                                $("#addCustomerStep1 #last_name").val('');
                                $("#addCustomerStep1 #email").val('');
                                $("#addCustomerStep1 #phone_number").val('');
                                $("#add-single-customer-next-step").attr('data-already-added', 'false');
                                $("#customizeReviewRequestsBtn").attr('data-customer-id', '');
                                $("#customizeReviewRequestsBtn").attr('data-varification-code', '');
                            }
                            // showPreloader();
                            // location.href = baseUrl + '/get-more-reviews';
                        });
    ...
    $("#add-single-customer-next-step").click(function(){
        ...
        if(alreadyAddedCheck=='true'){
            ...
            if (!(viewedSendReviewInviteSettings == 1)) {
                // console.log('here');
                $('#send_review_invite_view_report_link').click();
                var baseUrl = $('#hfBaseUrl').val();
    ...
    $(document).on('click',"#showAddMultipleCustomerStep3Modal",function () {
        ...
        {
            $("#addMultipleCustomerStep3 .step-body").append('<div class="validate-check col-sm-12">\n' +
                '                        <div class="checkbox checkbox-info p-t-0">\n' +
                '                            <input id="checkbox-signup" type="checkbox">\n' +
                '                            <label for="checkbox-signup" style="font-size: 15px">Duplicate addresses will be removed. We do not send confirmation emails to imported addresses and trust that you\'ve gathered proper permission to send to every address in your audience. I certify that all recipients have opted in to receive these communications. </label>\n' +
                '                        </div>\n' +
                '                    </div>');
        }
    ...
    $(document).on('click',"#reset-email-default-preview",function (e){
        ...
        // $('#email-preview-wrap .content-body').html('<p class="email-content" id="email-content">Hi {{name}}!\n' +
        //     'At '+businessName+', We are committed to offering the highest level of patient care in Boston.' +
    ...
    $(document).on('click',"#reset-sms-default-preview",function (e){
        ...
        $('#sms-preview-wrap .content-body').attr('data-default','true');
        // when reset default character limit applied
        displayCharacters("#sms-preview-wrap #sms-content", 'http://bit.ly/2LhkWmX');
    });

    $(document).on('click',".showCustomizeEmailModal",function (e){
        $(".thumbnail-up-down").hide();
        $("#showCustomizeEmailModal").click();
    ...
    $(document).on('click',"#showCustomizeEmailModal",function (e){
        ...
        if(checkDefault=='false'){
            ...
            $('#email-preview-wrap .content-body').html('<textarea id="customizedEmailMessage" maxlength="'+limitContent+'" class="form-control test-customized" data-placeholder="Enter Text Here..."></textarea>\n' +
            ...
            $('#email-preview-wrap .content-body').addClass('editor-mode');
            $('#email-preview-wrap .content-body').html('<textarea id="customizedEmailMessage" maxlength="'+limitContent+'" class="form-control" data-placeholder="Enter Text Here..."></textarea>\n' +
                '<div class="display-error"></div>'+
                '<div class="edit-icons">\n' +
                '  <a href="javascript:;" id="cancelCustomizedEmail"><span class="fa fa-remove"></span></a>\n' +
                '  <a href="javascript:;" id="updateCustomizedEmail"><span class="fa fa-check"></span></a>\n' +
            ...
            emailContent = emailContentMessage;
            $('#customizedEmailMessage').val(emailContent);
        }

        showCounter('#email-preview-wrap');

        refreshCounter("#customizedEmailMessage");

        displayValueCharacters("#email-preview-wrap #customizedEmailMessage");


    });

    $(document).on('click',".showCustomizeSMSModal",function (e){
        $("#showCustomizeSMSModal").click();
    });
    ...
    $(document).on('click',"#showCustomizeSMSModal",function (e){
        ...
        if(checkDefault=='false'){
            ...
            $('#sms-preview-wrap .content-body').html('<textarea id="customizedSMSMessage" maxlength="'+limitContent+'" class="form-control" data-placeholder="Enter Text Here..."></textarea>\n' +
                '<div class="edit-icons">\n' +
                ...
                '</div>\n' +
                '<div class="display-error" style="position: absolute;bottom: -50px;"></div>'
            );
            ...
            var smsContent = smsContentMessage;
            $('#customizedSMSMessage').val(smsContent);
        }

        showCounter('#sms-preview-wrap');

        refreshCounter("#customizedSMSMessage");

        displayValueCharacters("#sms-preview-wrap #customizedSMSMessage");

    });
// $('#customizedEmailMessage').keyup(function (e) {
//     var customizedEmailMessage= $('#customizedEmailMessage').val();
//         console.log(customizedEmailMessage.length);
// });

    displayCharacterLimitError('#email-preview-wrap', '#customizedEmailMessage');
    displayCharacterLimitError('#sms-preview-wrap', '#customizedSMSMessage');

    function displayCharacterLimitError(wrapper, input) {
        $(document.body).on('blur keyup', input, function()
        {
            var inputVal= $(input).val();
            // console.log(input);
            // console.log(inputVal.length);
            var display_error = wrapper+" .display-error";
            // textarea handler, and parent div
            updateCount(input, wrapper);

            console.log("limitContent");
            console.log(limitContent);

            if (inputVal.length > limitContent) {
                $(display_error).html('<span class="text-danger">'+limitContent+' characters limit exceeded.</span>');
            } else if (inputVal.length == limitContent) {
                $(display_error).html('<span class="text-secondary">'+limitContent+' characters limit Reached.</span>');
            } else {
                $(display_error).html('');
            }
...
// $('#customizedEmailMessage').keyup(function (e){ console.log($('#customizedEmailMessage').val().length); });
    $(document).on('click',"#updateCustomizedEmail",function (e){
        var customizedEmailMessage= $('#customizedEmailMessage').val();

        // console.log(customizedEmailMessage.length);

        if (customizedEmailMessage.length > limitContent) {
            $("#email-preview-wrap .display-error").html('<span class="text-danger">'+limitContent+' characters limit exceeded.</span>');
            return false;
        }
    ...
    $(document).on('click',"#updateCustomizedSMS",function (e){
        ...
        if (customizedSMSMessage.length > limitContent) {
            $("#sms-preview-wrap .display-error").html('<span class="text-danger">'+limitContent+' characters limit exceeded.</span>');
            return false;
        }
        ...
        if(customizedSMSMessage.length>limitContent){
            swal({
                title: "",
                text: "The text message has exceeded "+limitContent+" characters.",
                type: 'error',
                allowOutsideClick: false
            });
            return false;
    ...
    $(document).on('click',"#cancelCustomizedEmail",function (e){
        ...
        if(checkDefault=='false'){
            ...
            $('#email-preview-wrap .content-body').html('<p class="email-content" id="email-content">'
                +emailContentMessage+
                '</p>' +
                '<a id="showCustomizeEmailModal" href="javascript:;"><span class="mdi mdi-pencil"></span></a>'
            );
    ...
    $(document).on('click',"#cancelCustomizedSMS",function (e){
        ...
        $('#customizeReviewRequestsBtn').removeClass('disabled').removeAttr('disabled');
        // when cancel updating character limit apply
        displayCharacters("#sms-preview-wrap #sms-content", 'http://bit.ly/2LhkWmX');
    });
...
$(document).on('click',"#save_settings",function (e) {
    ...
    // var sending_option= $('#sending_option').val();
    // sending option 5 set to decide to system
    var sending_option = 5;
...
function showCounter(handler, count = 0) {
    // count = 0;
    // console.log("showCounter");
    // console.log($(".edit-icons", handler).length);
    $(".edit-icons", handler).prepend('<div class="char_count"><span class="char-limit">'+count+'</span></div>');
}

function updateCount(handler, parentHandler, count = '') {

    // take character value lengt
    count = $(handler).val().length;

    // show in ocunter.
    $(".char-limit", parentHandler).html(count);
}

function removeCounter(handler)
{
    $(".char_count", handler).remove();
}

function refreshCounter(handler) {
    setTimeout(function () {
        // console.log("handler");
        // console.log(handler);
        $(handler).blur();
...
function displayCharacters(handler, replace) {
    // console.log('displayCharacters');

    console.log("limitContent");
    console.log(limitContent);
    $(handler).text(function(index, currentText) {
        return currentText.substr(0, limitContent).replace(replace, '');
    });
 }
 function displayValueCharacters(handler) {
    // console.log('displayValueCharacters');
    $(handler).val(function(index, currentText) {
        return currentText.substr(0, limitContent);
    });
 }
...
function loadCustomerSettings()
{
    ...
    $.ajax({
        ...
        if(data)
        {
            // console.log('data');
            // console.log(data);
            window.reviewRequestSettingsData = data;
        }
    });
}

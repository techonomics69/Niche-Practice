...
$(document.body).on('click', '.connect-app' ,function() {
    ...
    if (actionType == 'zocdoc') {
        var userGuidanceMessage = 'Enter Your Profile URL to connect your app. <br /> Example URL: https://www.zocdoc.com/dentist/John-Smith-DDS';
    } else if (actionType == 'vitals') {
        var userGuidanceMessage = 'Enter Your Profile URL to connect your app. <br /> Example URL: https://www.vitals.com/dentists/Dr_John_Smith';
    } else if (actionType == 'ratemd') {
        var userGuidanceMessage = 'Enter Your Profile URL to connect your app. <br /> Example URL: https://www.ratemds.com/doctor-ratings/123456/Dr-Richard-Green-Miami-FL.html';
    } else if (actionType == 'yelp') {
        var userGuidanceMessage = 'Enter Your Profile URL to connect your app. <br /> Example URL: https://www.yelp.com/biz/john-smith-dds-miami-2';
    } else if (actionType == 'healthgrades') {
        var userGuidanceMessage = 'Enter Your Profile URL to connect your app. <br /> Example URL: https://www.healthgrades.com/dentist/dr-Joe-Smith-b3ar7e';
    } else if (actionType == 'google places') {
        var userGuidanceMessage = 'Enter Your Profile url to connect your app. By Connect your app here you can manage your reviews from here.';
    }
    ...
    if(actionType === 'facebook' || actionType === 'twitter')
    {
        ...
        html += '<h3 class="">Authorize NichePractice on '+originalAction+'</h3>';
        ...
        html += '<li>Receive viral content recommendations from NichePractice and share them to your '+originalAction+' page.</li>';

        if(actionType === 'facebook')
        {
            html += '<li>Get Reviews from your page.</li>';
        }
        html += '</ul>';
        html += '</div>';
        html += '</div>';
    ...
    else
    {
        ...
        html += '<div class="user-guidance-message">' + userGuidanceMessage + '</div>';
    ...
    if(currentPage === 'promotions')
    {
        if($(".social-connect-tabs").length == 0)
        {
            var socialConnection = '';

            socialConnection += '<ul style="display: block;" class="nav nav-tabs social-connect-tabs">\n' +
                '        <li class="facebook-tab active">\n' +
                '        <a type="javascript:void(0)" class="connect-app" data-type="Facebook">Connect Facebook</a>\n' +
                '    </li>\n' +
                '    <li class="twitter-tab"><a type="javascript:void(0)" class="connect-app" data-type="Twitter">Connect Twitter</a></li>\n' +
                '    </ul>';

            console.log("length checking " + $(".social-connect-tabs").length)

            $(".connect-app-interface .modal-header").prepend(socialConnection);
        }
    }

    $(".connect-app-interface .modal-header").after(html);
...
$(document.body).on('click', '#loginSocial', function() {
    ...
    if(type == 'facebook')
    {
        ...
        else if(currentPage === 'promotions')
        {
            ...
            // return false;
            location.href = actionApiUrl+'social-media/login?referType='+currentPage+'&business_id='+business_id+'&promotion='+window.templateId;
        }
        else
        {
...
function runSocialAction()
{
    ...
    if(accessToken == 1 && actionType === 'facebook')
    {
        ...
        html += '<div class="modal-body retrieb ">';
        html += '<div class="alert" style="display: none;"></div>';
        ...
        html += '<div class="modal-footer">';
        html += '<button type="button" class="btn btn-default btn-s-cancel" data-dismiss="modal">Cancel</button>';
        // html += '<input type="hidden" id="actionApiUrl" value="https://dev-api.netblaze.com/public/">';
        html += '</div>';
...
// var formData = false;
// if (window.FormData) formData = new FormData();
// formData.append('send', 'save-notifications-setting');
// formData.append('notification-email',notificationEmail);
// formData.append('notification-switch', switchVal);
// $.ajax({
//     headers: {
//         'X-CSRF-TOKEN': $('input[name="_token"]').val()
//     },
...
$(document.body).on('click','#saveNotification', function (e) {
    e.preventDefault();
    showPreloader();
    var baseUrl = $('#hfBaseUrl').val();
    ...
    var notificationSwitch = $('#checkboxCheck').prop('checked');
    console.log('notificationSwitch');
    console.log(notificationSwitch);
    ...
    var action = 'save-notifications-setting';
    $.ajax({
        headers: {
            'X-CSRF-TOKEN': $('input[name="_token"]').val()
        },
        ...
        else if (statusCode == 404 || statusCode == 1) {
            swal({
                title: "",
                text: statusMessage,
                type: "error"
...
function manualConnectBusiness()
{
    if (!errorFound) {
        ...
        $.ajax({
            ...
            if(!(currentPage === 'promotions' && type === 'facebook'))
            {
                hidePreloader();
            }
            ...
            if (statusCode === 200) {
                ...
                if(currentPage === 'promotions' && type === 'facebook')
                {
                    ...
                    window.socialMediaPostsData = {'Facebook': data};
                    var socialPageName = data.name;

                    $("#flag").val('sharing');

                    $(".select-social-media-buttons-container").html('' +
                        '<button type="button" data-type="Facebook" class="btn select-social-media-button facebook-social-media-button selected-social-media">\n' +
...
$(document).on('click', '.social-connect-tabs li', function(){
    $(".social-connect-tabs li").removeClass('active');

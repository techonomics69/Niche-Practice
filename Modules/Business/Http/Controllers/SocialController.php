...
//        $businessId = $businessResult['business_id'];
        ...
        try {
            ...
            $after = $request->after ? $request->after : '';
            $socialMediaPostsData = $this->socialEntity->getSocialMediaPosts($data, $limit, $after);
...
//            log::info('these are social posts');
//            log::info($this->data['socialMediaPostsData']);
            // $this->data['socialMediaPostsData']['published']['Facebook']['posts']
            // Log::info("test Name " . $this->data['socialMediaPostsData']['Facebook']['name']);
            // dd( $this->data );
    ...
    public function nextSocialPosts(Request $request)
    {
        # code...
        $businessResult = $this->businessEntity->userSelectedBusiness();
        ...
        catch(Exception $e)
        {
            Log::info("socialPosts -> " . $e->getMessage());
            $this->data['message'] = 'Problem in retrieving Posts Page. Please try again later';
            return response()->json($this->data['message']);
        }
    }
    public function getSocialMediaPost(Request $request)
    {
        return $this->socialEntity->getSocialMediaPost($request);
    }
...
//                        Log::info("in > " . $socialData['access_token']);
        ...
        catch(Exception $e)
        {
            ...
            $statusData = [
                'status_code' => $responseCode,
                'status_message' => 'An unknown error has occurred. Please try again.'
            ];
        }
    ...
    public function removeAccessToken($google = '')
    {
//        log::info('coming to removeAccessToken');
//        log::info($google);
//        log::info($googleAnalytics);

        if($google = 'googleAnalytics'){
            $this->sessionService->setOAuthToken(['analyticsAccessToken' => '']);
        }
        else{
            $this->sessionService->setOAuthToken(['businessAccessToken' => '']);
        }
    ...
    {
        return $this->socialEntity->deletePost($request);
    }
...
//        return $responseData;
    ...
    public function testingfbapi()
    {
        # code...
        return $this->facebookEntity->nextPosts();
    }

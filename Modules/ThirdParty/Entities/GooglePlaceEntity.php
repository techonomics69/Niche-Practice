    ...
    public function getFirstPlaceID($request)
    {
        ...
        if ($searchFor != '') {
            try {
                ...
                Log::info('googlePlaceResultDetail in automatic connect for api');
...
//            $detailUrl = ( $appEnvironment == 'production') ? Config::get('custom.yelpProdManualConnect'): Config::get('custom.yelpTestManualConnect');
            $backupMode = Config::get('apikeys.backupMode');
            Log::info("backupMode");
            Log::info($backupMode);
            $serverUrl = ( $backupMode == 'live' ) ? Config::get('custom.PEN_Scrapper_Prod_SERVER_URL') : Config::get('custom.SERVER_URL');

            $detailUrl = ( $backupMode == 'live' ) ? Config::get('custom.googleProdManualConnect') : Config::get('custom.landingGoogleTestManualConnect');

            $queryParams = ( $backupMode == 'live' ) ? $queryParams1 : $queryParams2;
//            $serverUrl = ( $appEnvironment == 'production') ? Config::get('custom.Scrapper_Prod_SERVER_URL'): Config::get('custom.SERVER_URL');
            ...
            if ($responseCode == 200) {
                ...
                if ($results['_metadata']['outcomeCode'] == 200)
                {
                    ...
                    if ($googleplaceresults)
                    {
                         ...
                         * Parse url from string and add http:// before website url
                         */
//                        $parseUrl = parse_url(trim($placeResultData['website']));
//
//                        if (!empty($parseUrl['host'])) {
//                            $url = 'http://' . $parseUrl['host'];
//                        }
//                        $placeResultData['website']=$url;
    ...
    public function updateGooglePlacesMaster($request)
    {
        ...
        try
        {
             ...
             * not by   >>>> " Business URl"
             */
            Log::info($request->get('targetUrl'));
            Log::info('request get  targetUrl');
...
//                        //   return $placeid;
//                        $request->merge(array('placeid' => $placeid));
//                        $result = $this->getPlaceResult($request);
//
//                        echo "updated";
//                        print_r($result);
//                        exit;
...
//                                $data['zipcode'] = !empty($records['zipcode']) ? $records['zipcode'] : '';
                            ...
                            Log::info("isNameChanged party");
                            Log::info($isNameChanged);
                            /**
...
//                                }
//
//                                return $this->helpReturn("Google Place Response.", $thirdPartyResult);
//                            }
                        }
                    }
...
//                dd("respo " . $responseCode);
    ...
    public function googleMyBusinessReviews($request)
    {
        //try{

        $businessUrl ='https://www.google.com/maps/place/B+%26+K+Tree+Removal+service/@38.799453,-90.494052,15z/data=!4m5!3m4!1s0x0:0xcdc4e3eaf03a7c00!8m2!3d38.799453!4d-90.494052';
        $client = new Client([]);
        ...
        if($response->getStatusCode() == 200)
        {
            ...
            if (!empty(($responseData['Results']['Name']))) {
                ...
                $reviews= $responseData['Results']['ReviewsDetail'];

                foreach($reviews as $fivestarReviews){

                    if($fivestarReviews['Rating'] =='5') {
                        $data['User Name']=$fivestarReviews['UserName'];
                        $data['Comment']=$fivestarReviews['Comment'];
                        $data['Rating']=$fivestarReviews['Rating'];
                        $data['Review Date']=$fivestarReviews['ReviewDate'];
                    }
                    $reviewsresult[] = $data;
                }

                return $this->helpReturn("Google My Business Response", $reviewsresult);
            }
        }
...
//        }
//        catch (Exception $e) {
//            Log::info("googleMyBusinessReviews > Google My Business Reviews >> " . $e->getMessage());
//            return $this->helpError(1, 'An unknown error has occurred. Please try again.');
//        }
    }

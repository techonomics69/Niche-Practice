...
//        $this->ChatHistoryEntity = new ChatHistoryEntity();
    ...
    public function getBusinessDetail(Request $request)
    {
        try {
            ...
            $query = ['Keyword' => $businessKeyword, 'key' => 'A8B9C31F69F12', 'pagination' => 'true', 'paginationLimit' => '1'];
...
//
            ...
            $serverUrl = ( $appEnvironment == 'production') ? Config::get('custom.PEN_Scrapper_Prod_SERVER_URL'): Config::get('custom.PEN_SERVER_URL');
...
//            exit;
            ...
            if ($type != '' && $type != 'all') {
                ...
                foreach ($data as $row) {
                    ...
                    if (strtolower($type) == 'yelp') {
                        $result = $yelpEntity->getBusinessUrlHistoricalDetail($pageUrl);
                    }
                    if (strtolower($type) == 'tripadvisor') {
                        $result = $tripEntity->getBusinessUrlHistoricalDetail($pageUrl);
                    }
                ...
                foreach ($data as $row) {
                    ...
                    if (strtolower($thirdPartyType) == 'tripadvisor') {
                        $result = $tripEntity->getBusinessUrlHistoricalDetail($pageUrl);
                    }
                    else if (strtolower($thirdPartyType) == 'yelp') {
                        $result = $yelpEntity->getBusinessUrlHistoricalDetail($pageUrl);
                    }
                    else if (strtolower($thirdPartyType) == 'google places') {
                        $result = $googleEntity->getBusinessUrlHistoricalDetail($pageUrl);
                    }
        ...
        try {
            ...
            if (empty($request->get('targetUrl'))) {
                ...
                if ($responseCode == 200) {
                    ...
                    Log::info('netblaze (ta) ka business name' . $request->businessKeyword);

//                    $comparison = compareStringGetSubSetResult($records['Name'], $request->businessKeyword);
//
//                    if ($comparison == 'false') {
//                        Log::info('tripadvisor fail hogaya');
//                        $businessId = $request->get('business_id');
...
//                    }
                    ...
                    if ($records) {
                        ...
                        if ($thirdPartyId && $isNameChanged) {
                            ...
                            if (!empty($userReviews) && !empty($request->get('targetUrl'))) {
                                $this->storeUserReviews($userReviews, $thirdPartyId, ucfirst($data['type']), $request);
                            }
                        }
                        if (empty($request->get('targetUrl'))) {
                            return $this->helpReturn("Data saved.", $tripAdvisorResult);
                        } else {
                            $businessIssuesData = $businessObj->businessIssues($request, $businessId);
                            if ($businessIssuesData['_metadata']['outcomeCode'] == 200) {
                                $tripAdvisorResult['businessIssues'] = $businessIssuesData['records'];
                            }
    ...
    public function notificationsSentToUsers($thirdPartyId, $user, $pageUrl)
    {
        try {
            /**
             * Retrieve user from token or id
             */
            $userData = User::find($user);
            $notificationsNotSendRecords = TripadvisorReview::where(
                [
                    'third_party_id' => $thirdPartyId,
            ...
            $url = '';
            $notificationcount = $notificationsNotSendRecords->count();
            if ($notificationcount > 0) {
                $foo = [];
                $foo = new PushNotification();
                $deviceToken = $userData->device_id;
                /**********************Start Save notification in chat**********************/
                $data['sender'] = 1; // currently sender is an system user.
                $data['receiver'] = $user; // currently receiver is an agent.
                $url = $pageUrl;
                $data['message'] = 'You have received new review(s) on TripAdvisor. Tap here to read it now. >>' . $url;
                ChatHistory::create($data);
                /**********************End Save notification in chat**********************/
                if ($deviceToken != '') {
                    $foo->app('appNameIOS')
                        ->to($deviceToken)
                        ->send('You just got a new review(s) on Tripadvisor. Read it now.');
                }
                $notificationsNotSendRecords->update(['is_notification_send' => 1]);
            }
        } catch (Exception $e) {
        ...
        try {
            ...
            foreach ($result as $review) {
                if ($job == 'cronjob' && $type == 'Google Places') {
                    $tripRecord = '';
                    $tripRecord = TripadvisorReview::where('third_party_id', $reviewsLinkedWith)
                        ->where('type', 'Google Places')
                        ->where('review_url', $review['ReviewURL'])
                        ->first();
...
//                        }
                ...
                elseif($type == 'Zocdoc' || $type == 'Healthgrades' || $type == 'Ratemd') {
                    ...
                    if(!empty(getIndexedvalue($review, 'author')))
                    {
                        $reviewData[$i]['reviewer'] = getIndexedvalue($review, 'author');
                    }
                    elseif(!empty(getIndexedvalue($review, 'UserName')))
                    {
                        $reviewData[$i]['reviewer'] = getIndexedvalue($review, 'UserName');
                    }
                    else
                    {
                        $reviewData[$i]['reviewer'] = '';
                    }

                    $reviewData[$i]['message'] = getIndexedvalue($review, 'Comment');

//                    Log::info("message ");
//                    Log::info($reviewData[$i]['message']);

//                    $reviewData[$i]['review_url'] = $review['ReviewURL'];

                    if(!empty(getIndexedvalue($review, 'overall')))
                    {
                        $reviewData[$i]['rating'] = getIndexedvalue($review, 'overall');
                    }
                    elseif(!empty(getIndexedvalue($review, 'Rating')))
                    {
                        $reviewData[$i]['rating'] = getIndexedvalue($review, 'Rating');
                    }
                    else
                    {
                        $reviewData[$i]['rating'] = '';
                    ...
                    if(!empty(getIndexedvalue($review, 'date')))
                    {
                        $reviewData[$i]['review_date'] = getIndexedvalue($review, 'date');
                    }
                    elseif(!empty(getIndexedvalue($review, 'ReviewDate')))
                    {
                        $reviewData[$i]['review_date'] = getIndexedvalue($review, 'ReviewDate');
                    }
                    else
                    {
                        $reviewData[$i]['review_date'] = '';
                    }
...
//            return false;
            ...
            if (isset($firstIdArray[0])) {
                ...
                //case for yelp ,tripadvisor
                $result = bulk_insert("third_party_review", $reviewData);
                Log::info('Yelp, Tripadvisro last inserted id');
                Log::info($result);
            } else {
                $result = 0;
                Log::info('Default inserted id');
                Log::info($result);
            }
            ...
            Log::info("result of type $type > " . $result);

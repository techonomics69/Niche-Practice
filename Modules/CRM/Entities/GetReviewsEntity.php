...
class GetReviewsEntity extends AbstractEntity
{
    ...
    public function getRecipientsList($request)
    {
        ...
        try {
            ...
            $recipients = DB::table('recipients')
                ->join('review_requests', 'recipients.id', '=', 'review_requests.recipient_id')
                ->select('recipients.first_name', 'recipients.email', 'recipients.last_name', 'review_requests.recipient_id',DB::raw('DATE_FORMAT(review_requests.date_sent, "%m-%d-%Y") as date_sent'), 'review_requests.site', 'review_requests.type', 'recipients.smart_routing', 'recipients.phone_number')
                ->where('review_requests.message', '=', null)
    ...
    public function updateSites(Request $request)
    {
        ...
        try {
            ...
            if (filter_var($request->email, FILTER_VALIDATE_EMAIL)) {
                Log::info('email');
                $recipient = Recipient::where('email', $request->email)->where('varification_code', $request->secret)->first(); //check autheticated user
                $Recordtype = 'email'; //for example SMS , Email
            } else if (!filter_var($request->email, FILTER_VALIDATE_EMAIL)) {
                $recipient = Recipient::where('phone_number', $request->email)->where('varification_code', $request->secret)->first(); //check autheticated user
    ...
    public function saveFeedback($request)
    {
        ...
        try {
            ...
            $recipient = '';
            $business = Business::select('business_id','practice_name')
                        ->where('business_id','=',$request->id)->first();

            $arr = ['business_name' => $business['practice_name']];

            if (filter_var($request->email, FILTER_VALIDATE_EMAIL)) {
                $emailAll = Recipient::where('varification_code', $request->secret)->first();
                ...
                $mobileAll = Recipient::where('varification_code', $request->secret)->first();
            ...
            if (!empty($recipient)) {
                ...
                if (empty($request->tumb_status)) {
                    ...
                    Log::info($business['practice_name']);
                ...
                if ($request->tumb_status == 'down') { //if thumbs down then simply we save messagen , first name and date sent
                    ...
                    if ($settings->smart_routing == 'enable' || $settings->smart_routing == 'Enable') {
                        ...
                        $flag = true;
//                        $reviewRequest = ReviewRequest::where('recipient_id','=',$recipient->id)->where('id','=',$request->review_id)->first();
//
//                        $Reviewtype = getThirdPartyTypeShortToLongForm($reviewRequest->site);
//                        Log::info('check type');
//                        Log::info($Reviewtype);
//                        $thirdPartyMaster = TripadvisorMaster::select('third_party_id', 'business_id', 'type', 'average_rating', 'add_review_url', 'review_count')->where('type', $Reviewtype)->where('business_id', $business['business_id'])->whereNotNull('name')->get()->toArray();
//                        $socialMediaMaster = SocialMediaMaster::select('id as third_party_id', 'type', 'add_review_url', 'id', 'average_rating', 'page_reviews_count as review_count')
//                            ->where('type', $Reviewtype)->where('business_id', $business['business_id'])->whereNotNull('name')->get()->toArray();
//                        $mergeArray = array_merge($thirdPartyMaster, $socialMediaMaster); //merge both array
//                        Log::info('check merge array');
//                        Log::info($mergeArray);
                        $redirectUrl = $mergeArray;
                        if(empty($redirectUrl)){
                            return $this->helpError(3, 'You are not authorize for this action please click this link from your email.');
...
//                    if ($recipient->smart_routing == 'enable') {
//                        $flag = true;
//                    }
//                    Log::info('all details before enter in smart Routing');
//                    Log::info($business->business_id);
...
//                    $redirectUrl = $this->crmEntity->smartRouting($business->business_id, $recipient->smart_routing, $recipient->id, [], $flag);
    ...
    public function emailSending($record, $request, $user, $businessId, $businessName, $Useremail, $formatedDate)
    {
        ...
        $data['varification_code'] = randomString();
        ...
        if ($request->smart_routing == 'enable') {
            ...
            if ($finalRedirectUrlArray['_metadata']['outcomeCode'] == 200) {
                ...
                try {
                    ...
                    Mail::to($request->email)->send(new CreateAddReciepentsEmail($request->first_name, $businessName, $data['varification_code'], $request->email, $Useremail));
    ...
    public function sendSms($phoneRecords, $request, $user, $businessId, $businessName, $Useremail, $formatedDate, $typeFlag)
    {
        ...
        $url = Bitly::getUrl($encodedurl . '/business-review/' . $filterPhoneNumber . '/' . $data['varification_code'] . '/' . $businessName);
...
//        }
    ...
    public
    function emailTesting($request)
    {
        // try {
        Mail::to($request->email)->send(new CreateAddReciepentsEmail('first name', 'test business name', 'adfjljl', $request->email, 'test@gmail.com'));
